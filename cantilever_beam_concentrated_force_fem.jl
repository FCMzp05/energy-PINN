...
Base.@kwdef mutable struct Config
    length::Float64 = 60.0
    height::Float64 = 20.0
    young::Float64 = 200000.0
    poisson::Float64 = 0.30
    thickness::Float64 = 1.0
    point_force_y::Float64 = -200.0
    nx::Int = 20
    ny::Int = 15
    output_dir::String = ""
end
...
function parse_cli!(cfg::Config)
    ...
    if isempty(cfg.output_dir)
        cfg.output_dir = joinpath(@__DIR__, "outputs", "cantilever_beam_concentrated_force_fem")
    end
    return cfg
end

function boundary_sets(cfg::Config, nodes::Matrix{Float64})
    tol = 1.0e-10
    left = findall(i -> abs(nodes[i, 1]) < tol, 1:size(nodes, 1))
    right = findall(i -> abs(nodes[i, 1] - cfg.length) < tol, 1:size(nodes, 1))
    return left, right
end

function build_smoothed_point_load(cfg::Config, nodes::Matrix{Float64}, right_nodes::Vector{Int})
    ndof = 2 * size(nodes, 1)
    fext = zeros(Float64, ndof)
    ymid = 0.5 * cfg.height
    order = sort(right_nodes; by = nid -> abs(nodes[nid, 2] - ymid))
    selected = order[1:min(3, length(order))]
    weights = [0.5, 0.3, 0.2][1:length(selected)]
    weights ./= sum(weights)
    for (nid, w) in zip(selected, weights)
        fext[2 * nid] += cfg.point_force_y * w
    end
    return fext
end
...
function main()
    ...
    mesh = build_q4_mesh(cfg.length, cfg.height, cfg.nx, cfg.ny)
    left_nodes, right_nodes = boundary_sets(cfg, mesh.nodes)
    b_mats, detjw = build_q4_operators(mesh)
    dof_map = build_dof_map(mesh.quads)
    dmat = plane_strain_matrix(cfg.young, cfg.poisson)
    dmat_e = repeat(reshape(dmat, 1, 3, 3), size(mesh.quads, 1), 1, 1)
    stiff = assemble_stiffness(size(mesh.nodes, 1), dof_map, b_mats, detjw, dmat_e, cfg.thickness)
    fext = build_smoothed_point_load(cfg, mesh.nodes, right_nodes)
    ...
    save_fields_csv(joinpath(cfg.output_dir, "cantilever_beam_concentrated_force_fem_fields.csv"), mesh.nodes, fields)
    save_quads_csv(joinpath(cfg.output_dir, "cantilever_beam_concentrated_force_fem_mesh.csv"), mesh.quads)
    save_boundary_csv(joinpath(cfg.output_dir, "cantilever_beam_concentrated_force_fem_boundary_left.csv"), left_nodes)
    save_boundary_csv(joinpath(cfg.output_dir, "cantilever_beam_concentrated_force_fem_boundary_right.csv"), right_nodes)
    open(joinpath(cfg.output_dir, "cantilever_beam_concentrated_force_fem_load.csv"), "w") do io
        println(io, "dof,value")
        for dof in findall(!iszero, fext)
            @printf(io, "%d,%.12e\n", dof, fext[dof])
        end
    end
    write_run_config(
        joinpath(cfg.output_dir, "cantilever_beam_concentrated_force_fem_run_config.txt"),
        [
            "length" => cfg.length,
            "height" => cfg.height,
            "young" => cfg.young,
            ...
            "point_force_y" => cfg.point_force_y,
            "nx" => cfg.nx,
            "ny" => cfg.ny,
        ],
    )
