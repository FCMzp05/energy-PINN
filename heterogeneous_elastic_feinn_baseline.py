...
class Config:
    ...
    height: float = 5.0
    thickness: float = 1.0
    inclusion_center_x: float = 2.5
    inclusion_center_y: float = 2.5
    inclusion_radius: float = 1.0
    young_matrix: float = 1.0
    young_inclusion: float = 10.0
    poisson: float = 0.30
...
def make_q4_mesh(cfg: Config) -> dict[str, np.ndarray]:
    ...
    for j in range(cfg.ny):
        for i in range(cfg.nx):
            ...
            center = nodes[quad].mean(axis=0)
            r2 = (center[0] - cfg.inclusion_center_x) ** 2 + (center[1] - cfg.inclusion_center_y) ** 2
            material_ids.append(1 if r2 <= cfg.inclusion_radius**2 else 0)
...
def gauss_rule_q4() -> tuple[np.ndarray, np.ndarray]:
    a = 1.0 / np.sqrt(3.0)
    pts = np.array([[-a, -a], [a, -a], [a, a], [-a, a]], dtype=np.float64)
...
def build_element_dmat(cfg: Config, material_ids: np.ndarray) -> np.ndarray:
    d_matrix = plane_strain_matrix(cfg.young_matrix, cfg.poisson)
    d_inclusion = plane_strain_matrix(cfg.young_inclusion, cfg.poisson)
    out = np.zeros((material_ids.shape[0], 3, 3), dtype=np.float64)
    out[material_ids == 0] = d_matrix
    out[material_ids == 1] = d_inclusion
    return out
...
def build_external_force(cfg: Config, nodes: np.ndarray, right_edges: np.ndarray) -> np.ndarray:
    ...
    for edge in right_edges:
        ...
        for s, w in zip(gp, gw):
            ...
            nmat = np.array([[n[0], 0.0, n[1], 0.0], [0.0, n[0], 0.0, n[1]]], dtype=np.float64)
...
def boundary_data(nodes: np.ndarray) -> dict[str, np.ndarray]:
    tol = 1.0e-8
...
def apply_hard_bc(cfg: Config, nodes_t: torch.Tensor, raw_out: torch.Tensor) -> torch.Tensor:
    ...
    # The forward FEINN paper states the left side is clamped.
    # Only impose the left clamp here and leave top/bottom free.
    bx = xh
...
def build_free_dofs(nodes: np.ndarray) -> np.ndarray:
    binfo = boundary_data(nodes)
    fixed = set()
    for nid in binfo["left"]:
        fixed.add(2 * int(nid))
...
def train_feinn(
    ...
    free_dofs = torch.tensor(build_free_dofs(nodes), dtype=torch.long, device=tdata["nodes"].device)
...
def draw_circle(ax: plt.Axes, cfg: Config) -> None:
    circle = plt.Circle((cfg.inclusion_center_x, cfg.inclusion_center_y), cfg.inclusion_radius, facecolor="none", edgecolor="#444444", linewidth=0.7)
    ax.add_patch(circle)
...
def plot_panel(
    ...
    names = [("ux", r"$u_x$"), ("uy", r"$u_y$")]
    fig, axes = plt.subplots(len(names), 3, figsize=(9, 6), dpi=220)
...
def write_mesh_csv(nodes: np.ndarray, quads: np.ndarray, material_ids: np.ndarray, out_path: Path) -> None:
    ...
    for e, q in enumerate(quads):
        rows.append({"element_id": e, "n1": int(q[0]), "n2": int(q[1]), "n3": int(q[2]), "n4": int(q[3]), "material_id": int(material_ids[e])})
    pd.DataFrame(rows).to_csv(out_path, index=False)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Reproduce the FEINN heterogeneous elastic example.")
...
def main() -> None:
    ...
    out_dir = Path(args.output_dir) if args.output_dir else base_dir / "outputs" / f"heterogeneous_elastic_feinn_baseline_{objective_suffix(cfg.objective_mode)}"
    ...
    print(f"[Mesh] nodes={nodes.shape[0]} | quads={quads.shape[0]} | right_edges={right_edges.shape[0]}")
    ...
    write_mesh_csv(nodes, quads, material_ids, out_dir / "heterogeneous_elastic_mesh.csv")
    save_fields_csv(nodes, material_ids, quads, fem_fields, feinn_fields, out_dir / "heterogeneous_elastic_fields.csv")
    history.to_csv(out_dir / "heterogeneous_elastic_training_history.csv", index=False)
    plot_loss(history, out_dir / "heterogeneous_elastic_loss.png")
    plot_panel(cfg, nodes, quads, fem_fields, feinn_fields, out_dir / "heterogeneous_elastic_panel.png")
    with open(out_dir / "heterogeneous_elastic_metrics.json", "w", encoding="utf-8") as f:
        json.dump(summarize_metrics(fem_fields, feinn_fields), f, indent=2)
    with open(out_dir / "heterogeneous_elastic_run_config.json", "w", encoding="utf-8") as f:
        json.dump(asdict(cfg), f, indent=2)

    print(f"Output directory: {out_dir}")
    print("Saved:")
    print("  heterogeneous_elastic_mesh.csv")
    print("  heterogeneous_elastic_fields.csv")
    print("  heterogeneous_elastic_training_history.csv")
    print("  heterogeneous_elastic_loss.png")
    print("  heterogeneous_elastic_panel.png")
    print("  heterogeneous_elastic_metrics.json")
    print("  heterogeneous_elastic_run_config.json")
