...
Base.@kwdef mutable struct Config
    ...
    interface_y::Float64 = 5.0
    young_top::Float64 = 30.0
    poisson_top::Float64 = 0.40
    young_bottom::Float64 = 50.0
    poisson_bottom::Float64 = 0.30
    traction_y::Float64 = -2.0
    nx::Int = 40
    ny::Int = 20
    output_dir::String = ""
end
...
function parse_cli!(cfg::Config)
    ...
    if isempty(cfg.output_dir)
        cfg.output_dir = joinpath(@__DIR__, "outputs", "multimaterial_elastic_fem")
    end
    return cfg
end
...
function main()
    ...
    dmat_top = plane_strain_matrix(cfg.young_top, cfg.poisson_top)
    dmat_bottom = plane_strain_matrix(cfg.young_bottom, cfg.poisson_bottom)
    material_ids = zeros(Int, size(mesh.quads, 1))
    dmat_e = zeros(Float64, size(mesh.quads, 1), 3, 3)
    top_edges_buf = Vector{NTuple{2, Int}}()
    for e in 1:size(mesh.quads, 1)
        q = mesh.quads[e, :]
        cy = mean(mesh.nodes[q, 2])
        if cy >= cfg.interface_y
            material_ids[e] = 1
            dmat_e[e, :, :] .= dmat_top
        else
            dmat_e[e, :, :] .= dmat_bottom
        end
        if abs(mesh.nodes[q[3], 2] - cfg.height) < 1.0e-10 && abs(mesh.nodes[q[4], 2] - cfg.height) < 1.0e-10
            push!(top_edges_buf, (q[4], q[3]))
        end
    end
    top_edges = zeros(Int, length(top_edges_buf), 2)
    for (i, edge) in enumerate(top_edges_buf)
    ...
    for edge in eachrow(top_edges)
        n1, n2 = edge
        p1 = view(mesh.nodes, n1, :)
        p2 = view(mesh.nodes, n2, :)
        length_edge = norm(p2 - p1)
        fe = cfg.thickness * length_edge / 2.0 .* [0.0, cfg.traction_y, 0.0, cfg.traction_y]
        ids = [2 * n1 - 1, 2 * n1, 2 * n2 - 1, 2 * n2]
        fext[ids] .+= fe
    end

    left_nodes = findall(i -> abs(mesh.nodes[i, 1]) < 1.0e-10, 1:size(mesh.nodes, 1))
    right_nodes = findall(i -> abs(mesh.nodes[i, 1] - cfg.width) < 1.0e-10, 1:size(mesh.nodes, 1))
    bottom_nodes = findall(i -> abs(mesh.nodes[i, 2]) < 1.0e-10, 1:size(mesh.nodes, 1))
    bc = Dict{Int, Float64}()
    ...
    save_fields_csv(joinpath(cfg.output_dir, "multimaterial_elastic_fem_fields.csv"), mesh.nodes, fields)
    save_quads_csv(joinpath(cfg.output_dir, "multimaterial_elastic_fem_mesh.csv"), mesh.quads; material_ids = material_ids)
    save_boundary_csv(joinpath(cfg.output_dir, "multimaterial_elastic_fem_boundary_left.csv"), left_nodes)
    save_boundary_csv(joinpath(cfg.output_dir, "multimaterial_elastic_fem_boundary_right.csv"), right_nodes)
    save_boundary_csv(joinpath(cfg.output_dir, "multimaterial_elastic_fem_boundary_bottom.csv"), bottom_nodes)
    save_edges_csv(joinpath(cfg.output_dir, "multimaterial_elastic_fem_top_edges.csv"), top_edges)
    write_run_config(
        joinpath(cfg.output_dir, "multimaterial_elastic_fem_run_config.txt"),
        [
            "width" => cfg.width,
            "height" => cfg.height,
            "interface_y" => cfg.interface_y,
            "young_top" => cfg.young_top,
            "poisson_top" => cfg.poisson_top,
            "young_bottom" => cfg.young_bottom,
            "poisson_bottom" => cfg.poisson_bottom,
            "traction_y" => cfg.traction_y,
            "nx" => cfg.nx,
            "ny" => cfg.ny,
        ],
    )
