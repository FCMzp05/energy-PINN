...
def build_element_operators(nodes: np.ndarray, elements: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    for xi, eta in gauss_points:
        ...
        b_mats.append(b)
        detjs.append(detj)
    return np.stack(b_mats), np.array(detjs, dtype=np.float64)
...
def boundary_sets(cfg: Config, nodes: np.ndarray) -> dict[str, np.ndarray]:
    ...
    mid_right = right[np.argmin(np.abs(nodes[right, 1] - 0.5 * cfg.height))]
    return {"left": left, "right": right, "mid_right": np.array([mid_right], dtype=np.int64)}


def build_smoothed_point_load(cfg: Config, nodes: np.ndarray) -> np.ndarray:
    bsets = boundary_sets(cfg, nodes)
    right = bsets["right"]
    y_mid = 0.5 * cfg.height
    order = np.argsort(np.abs(nodes[right, 1] - y_mid))
    selected = right[order[:3]]
    # Spread the concentrated load over a short boundary segment to avoid an overly singular nodal target.
    weights = np.array([0.5, 0.3, 0.2], dtype=np.float64)
    weights = weights / weights.sum()
    force = np.zeros(nodes.shape[0] * 2, dtype=np.float64)
    for nid, w in zip(selected, weights):
        force[2 * int(nid) + 1] += cfg.point_force_y * w
    return force
...
def solve_fem(cfg: Config, nodes: np.ndarray, elements: np.ndarray, b_mats: np.ndarray, detjs: np.ndarray, dof_map: np.ndarray) -> np.ndarray:
    ...
    force = build_smoothed_point_load(cfg, nodes)

    bc = {}
    for nid in boundary_sets(cfg, nodes)["left"]:
        bc[2 * nid] = 0.0
        bc[2 * nid + 1] = 0.0
    ...
    rhs = force[free] - stiff[np.ix_(free, fixed)] @ u[fixed]
...
def normalize_coords(cfg: Config, nodes_t: torch.Tensor) -> torch.Tensor:
    x = nodes_t.clone()
    x[:, 0] = 2.0 * nodes_t[:, 0] / cfg.length - 1.0
    x[:, 1] = 2.0 * nodes_t[:, 1] / cfg.height - 1.0
    return x


def apply_hard_bc(cfg: Config, nodes_t: torch.Tensor, raw_out: torch.Tensor) -> torch.Tensor:
    xhat = nodes_t[:, 0:1] / cfg.length
    # Concentrated force induces stronger localization than distributed displacement.
    # A slightly richer envelope improves expressiveness while preserving the fixed left boundary.
    ux = xhat * (0.04 + 0.04 * xhat) * raw_out[:, 0:1]
    uy = xhat * (0.12 + 0.28 * xhat + 0.10 * xhat * xhat) * raw_out[:, 1:2]
    return torch.cat([ux, uy], dim=1)


def build_torch_data(cfg: Config, nodes: np.ndarray, b_mats: np.ndarray, detjs: np.ndarray, dof_map: np.ndarray) -> dict[str, torch.Tensor]:
    dtype = getattr(torch, cfg.dtype)
    device = torch.device(cfg.device)
    external_force = build_smoothed_point_load(cfg, nodes)
    return {
        ...
        "fext": torch.tensor(external_force, dtype=dtype, device=device),
    }
...
def energy_scale(cfg: Config) -> float:
    return max(abs(cfg.point_force_y) * cfg.length, 1.0)
...
def train_feinn(cfg: Config, nodes: np.ndarray, elements: np.ndarray, b_mats: np.ndarray, detjs: np.ndarray, dof_map: np.ndarray) -> tuple[np.ndarray, pd.DataFrame]:
    tdata = build_torch_data(cfg, nodes, b_mats, detjs, dof_map)
...
def plot_panel(cfg: Config, nodes: np.ndarray, fem_fields: dict[str, np.ndarray], feinn_fields: dict[str, np.ndarray], out_path: Path) -> None:
    ...
    for i, key in enumerate(names):
        ...
        for j, data in enumerate([fem, pred, err]):
            ax = axes[i, j]
            im = ax.pcolormesh(xx, yy, data, shading="gouraud", cmap="coolwarm")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Reproduce the FEINN elastic cantilever beam under concentrated force with DEM/DCM objectives.")
...
def main() -> None:
    ...
    out_dir = Path(args.output_dir) if args.output_dir else base_dir / "outputs" / f"cantilever_beam_concentrated_force_feinn_baseline_{objective_suffix(cfg.objective_mode)}"
    out_dir.mkdir(parents=True, exist_ok=True)

    nodes, elements = build_mesh(cfg)
    print(f"[Mesh] nodes={nodes.shape[0]} | elements={elements.shape[0]}")
    b_mats, detjs = build_element_operators(nodes, elements)
    ...
    save_fields_csv(nodes, fem_fields, feinn_fields, out_dir / "cantilever_concentrated_force_fields.csv")
    history.to_csv(out_dir / "cantilever_concentrated_force_training_history.csv", index=False)
    plot_panel(cfg, nodes, fem_fields, feinn_fields, out_dir / "cantilever_concentrated_force_panel.png")

    with open(out_dir / "cantilever_concentrated_force_metrics.json", "w", encoding="utf-8") as f:
        json.dump(summarize_metrics(fem_fields, feinn_fields), f, indent=2)
    with open(out_dir / "cantilever_concentrated_force_run_config.json", "w", encoding="utf-8") as f:
        json.dump(asdict(cfg), f, indent=2)

    print(f"Output directory: {out_dir}")
    print("Saved:")
    print("  cantilever_concentrated_force_fields.csv")
    print("  cantilever_concentrated_force_training_history.csv")
    print("  cantilever_concentrated_force_panel.png")
    print("  cantilever_concentrated_force_metrics.json")
    print("  cantilever_concentrated_force_run_config.json")
