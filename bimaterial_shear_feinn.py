...
def main() -> None:
    ...
    out_dir = Path(args.output_dir) if args.output_dir else base_dir / "outputs" / f"bimaterial_shear_feinn_{objective_suffix(cfg.objective_mode)}"
    ...
    write_mesh_csv(nodes, quads, material_ids, out_dir / "bimaterial_shear_mesh.csv")
    save_fields_csv(nodes, material_ids, quads, fem_fields, feinn_fields, out_dir / "bimaterial_shear_fields.csv")
    history.to_csv(out_dir / "bimaterial_shear_training_history.csv", index=False)
    plot_loss(history, out_dir / "bimaterial_shear_loss.png")
    plot_panel(cfg, nodes, quads, fem_fields, feinn_fields, out_dir / "bimaterial_shear_panel.png")
    with open(out_dir / "bimaterial_shear_metrics.json", "w", encoding="utf-8") as f:
        json.dump(summarize_metrics(fem_fields, feinn_fields), f, indent=2)
    with open(out_dir / "bimaterial_shear_run_config.json", "w", encoding="utf-8") as f:
        json.dump(asdict(cfg), f, indent=2)

    print(f"Output directory: {out_dir}")
    print("Saved:")
    print("  bimaterial_shear_mesh.csv")
    print("  bimaterial_shear_fields.csv")
    print("  bimaterial_shear_training_history.csv")
    print("  bimaterial_shear_loss.png")
    print("  bimaterial_shear_panel.png")
    print("  bimaterial_shear_metrics.json")
    print("  bimaterial_shear_run_config.json")
