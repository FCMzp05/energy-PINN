...
class Config:
    ...
    load_case: str = "right_edge_displacement"
    right_uy_start: float | None = -6.0e-3
    right_uy_end: float | None = -1.0e-2
    load_steps: int = 20
    fem_nx: int = 63
    fem_ny: int = 13
    width_nn: int = 24
...
def load_fem_dataset(fem_dir: Path) -> tuple[dict[str, np.ndarray], dict[str, np.ndarray], pd.DataFrame]:
    field_rows = pd.read_csv(fem_dir / "cantilever_beam_displacement_fem_fields.csv")
    elem_rows = pd.read_csv(fem_dir / "cantilever_beam_displacement_fem_elements.csv")
    ...
    mesh = {
        ...
        "left_nodes": load_node_ids(fem_dir / "cantilever_beam_displacement_fem_boundary_left.csv"),
        "right_nodes": load_node_ids(fem_dir / "cantilever_beam_displacement_fem_boundary_right.csv"),
        "top_nodes": load_node_ids(fem_dir / "cantilever_beam_displacement_fem_boundary_top.csv"),
        "bottom_nodes": load_node_ids(fem_dir / "cantilever_beam_displacement_fem_boundary_bottom.csv"),
    }
    top_load_path = fem_dir / "cantilever_beam_displacement_fem_top_load.csv"
    ...
    fem_history = pd.read_csv(fem_dir / "cantilever_beam_displacement_fem_history.csv")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Standalone cantilever-beam FEINN weak-form displacement baseline.")
...
def main() -> None:
    ...
    fem_dir = Path(args.fem_dir) if args.fem_dir else case_outputs_dir / "cantilever_beam_displacement_fem"
    out_dir = (
        Path(args.output_dir)
        if args.output_dir
        else case_outputs_dir / f"cantilever_beam_displacement_feinn_baseline_{objective_suffix(args.objective)}"
    )
    out_dir.mkdir(parents=True, exist_ok=True)

    run_cfg = load_run_config(fem_dir / "cantilever_beam_displacement_fem_run_config.txt")
    cfg = Config(
        fem_dir=str(fem_dir),
        ...
        load_start=float(run_cfg.get("load_start", 0.0)) if "load_start" in run_cfg else None,
        load_end=float(run_cfg.get("load_end", 0.0)) if "load_end" in run_cfg else None,
        load_case=str(run_cfg.get("load_case", "right_edge_displacement")),
    ...
    save_fields_csv(mesh, fem_fields, pred_fields, out_dir / "cantilever_beam_displacement_feinn_baseline_fields.csv")
    plot_fields(mesh, fem_fields, pred_fields, out_dir / "cantilever_beam_displacement_feinn_baseline_fields.png")
    save_loss_curve(history, out_dir / "cantilever_beam_displacement_feinn_baseline_loss.png")
    save_sampling_figure(mesh, out_dir / "cantilever_beam_displacement_feinn_baseline_sampling.png")
    history.to_csv(out_dir / "cantilever_beam_displacement_feinn_baseline_training_history.csv", index=False)
    fem_history.to_csv(out_dir / "cantilever_beam_displacement_feinn_baseline_fem_history.csv", index=False)
    with (out_dir / "cantilever_beam_displacement_feinn_baseline_metrics.json").open("w", encoding="utf-8") as f:
        json.dump(summarize_metrics(fem_fields, pred_fields), f, indent=2)
    with (out_dir / "cantilever_beam_displacement_feinn_baseline_run_config.json").open("w", encoding="utf-8") as f:
        json.dump(asdict(cfg), f, indent=2)

    print(f"Output directory: {out_dir}")
    print("Saved:")
    print("  cantilever_beam_displacement_feinn_baseline_fields.csv")
    print("  cantilever_beam_displacement_feinn_baseline_fields.png")
    print("  cantilever_beam_displacement_feinn_baseline_loss.png")
    print("  cantilever_beam_displacement_feinn_baseline_sampling.png")
    print("  cantilever_beam_displacement_feinn_baseline_training_history.csv")
    print("  cantilever_beam_displacement_feinn_baseline_metrics.json")
    print("  cantilever_beam_displacement_feinn_baseline_run_config.json")
