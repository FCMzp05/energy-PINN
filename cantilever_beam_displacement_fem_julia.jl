...
Base.@kwdef mutable struct Config
    ...
    load_start::Float64 = 0.0
    load_end::Float64 = 0.0
    right_uy_start::Float64 = -6.0e-3
    right_uy_end::Float64 = -1.0e-2
    load_steps::Int = 20
    load_case::String = "right_edge_displacement"
    fem_nx::Int = 63
    fem_ny::Int = 13
...
function parse_cli!(cfg::Config)
    ...
    if isempty(cfg.output_dir)
        cfg.output_dir = joinpath(@__DIR__, "outputs", "cantilever_beam_displacement_fem")
    end
    return cfg
end
...
function build_top_load_vector(mesh::MeshData)
    ndof = 2 * size(mesh.nodes, 1)
    return zeros(Float64, ndof)
end

function displacement_schedule(cfg::Config)
    if cfg.load_steps == 1
        return [cfg.right_uy_end]
    end
    return collect(range(cfg.right_uy_start, cfg.right_uy_end, length = cfg.load_steps))
end

function apply_prescribed_right_uy!(u_full::Vector{Float64}, mesh::MeshData, target_uy::Float64)
    for nid in mesh.right_nodes
        u_full[2 * nid] = target_uy
    end
end
...
function solve_fem_reference(cfg::Config, mesh::MeshData)
    ...
    disps = displacement_schedule(cfg)
    for (step_id, q) in enumerate(disps)
        apply_prescribed_right_uy!(u_full, mesh, q)
        ref_norm = max(abs(q) * sqrt(length(mesh.right_nodes)), 1.0)
        @printf("  FEM load step %02d/%02d: right_uy=%.6e\n", step_id, cfg.load_steps, q)
...
function save_plots_if_needed(cfg::Config, mesh::MeshData, fem::Dict{String, <:Any})
    ...
    savefig(fig, joinpath(cfg.output_dir, "cantilever_beam_displacement_fem_fields.png"))
    hist = fem["history"]
    ...
    savefig(hist_fig, joinpath(cfg.output_dir, "cantilever_beam_displacement_fem_history.png"))
    disps = displacement_schedule(cfg)
    load_fig = plot(1:length(disps), disps, marker = :circle, xlabel = "Load Step", ylabel = "Applied vertical displacement", title = "Displacement schedule", legend = false, size = (620, 420))
    savefig(load_fig, joinpath(cfg.output_dir, "cantilever_beam_displacement_fem_load.png"))
end
...
function main()
    ...
    save_fields_csv(joinpath(cfg.output_dir, "cantilever_beam_displacement_fem_fields.csv"), mesh, fem)
    save_elements_csv(joinpath(cfg.output_dir, "cantilever_beam_displacement_fem_elements.csv"), mesh)
    save_boundary_csv(joinpath(cfg.output_dir, "cantilever_beam_displacement_fem_boundary_left.csv"), mesh.left_nodes)
    save_boundary_csv(joinpath(cfg.output_dir, "cantilever_beam_displacement_fem_boundary_right.csv"), mesh.right_nodes)
    save_boundary_csv(joinpath(cfg.output_dir, "cantilever_beam_displacement_fem_boundary_top.csv"), mesh.top_nodes)
    save_boundary_csv(joinpath(cfg.output_dir, "cantilever_beam_displacement_fem_boundary_bottom.csv"), mesh.bottom_nodes)
    save_history_csv(joinpath(cfg.output_dir, "cantilever_beam_displacement_fem_history.csv"), fem["history"])
    save_top_load_csv(joinpath(cfg.output_dir, "cantilever_beam_displacement_fem_top_load.csv"), fem["top_load"])
    write_run_config(joinpath(cfg.output_dir, "cantilever_beam_displacement_fem_run_config.txt"), cfg)
