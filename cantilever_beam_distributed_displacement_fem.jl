...
function parse_cli!(cfg::Config)
    ...
    if isempty(cfg.output_dir)
        cfg.output_dir = joinpath(@__DIR__, "outputs", "cantilever_beam_distributed_displacement_fem")
    end
    return cfg
end
...
function main()
    ...
    save_fields_csv(joinpath(cfg.output_dir, "cantilever_beam_distributed_displacement_fem_fields.csv"), mesh.nodes, fields)
    save_quads_csv(joinpath(cfg.output_dir, "cantilever_beam_distributed_displacement_fem_mesh.csv"), mesh.quads)
    save_boundary_csv(joinpath(cfg.output_dir, "cantilever_beam_distributed_displacement_fem_boundary_left.csv"), left_nodes)
    save_boundary_csv(joinpath(cfg.output_dir, "cantilever_beam_distributed_displacement_fem_boundary_right.csv"), right_nodes)
    open(joinpath(cfg.output_dir, "cantilever_beam_distributed_displacement_fem_reaction.csv"), "w") do io
    ...
    write_run_config(
        joinpath(cfg.output_dir, "cantilever_beam_distributed_displacement_fem_run_config.txt"),
        [
            "length" => cfg.length,
            "height" => cfg.height,
            "young" => cfg.young,
