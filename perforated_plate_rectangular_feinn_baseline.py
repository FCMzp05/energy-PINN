...
def load_fem_dataset(fem_dir: Path) -> tuple[dict[str, np.ndarray], dict[str, np.ndarray]]:
    field_rows = pd.read_csv(fem_dir / "perforated_plate_rectangular_fem_fields.csv")
    elem_rows = pd.read_csv(fem_dir / "perforated_plate_rectangular_fem_elements.csv")
    ...
    mesh = {
        "nodes": nodes,
        "triangles": triangles,
        "left_nodes": load_node_ids(fem_dir / "perforated_plate_rectangular_fem_boundary_left.csv"),
        "bottom_nodes": load_node_ids(fem_dir / "perforated_plate_rectangular_fem_boundary_bottom.csv"),
        "top_nodes": load_node_ids(fem_dir / "perforated_plate_rectangular_fem_boundary_top.csv"),
        "right_nodes": load_node_ids(fem_dir / "perforated_plate_rectangular_fem_boundary_right.csv"),
        "hole_nodes": load_node_ids(fem_dir / "perforated_plate_rectangular_fem_boundary_hole.csv"),
        "top_edges": load_top_edges(fem_dir / "perforated_plate_rectangular_fem_top_edges.csv"),
    }
    fem_fields = {
...
def radial_return_torch(
    ...
    fy = vm_trial - (cfg.yield_stress + hmod * alpha_prev)
    elastic_mask = fy <= 0.0
    vm_safe = torch.clamp(vm_trial, min=1.0e-12)
    dgamma = fy / (3.0 * mu + hmod)
    dgamma = torch.clamp(dgamma, min=0.0)
...
def compute_internal_force(
    ...
    stress, eps_p_new, alpha_new = radial_return_torch(
        cfg,
        strain,
        eps_p_prev,
        alpha_prev,
    ...
    return fint, stress, eps_p_new, alpha_new
...
def evaluate_state(
    ...
    hardening_energy = 0.5 * data["hmod"] * alpha_new * alpha_new
    dissipation = voigt_inner(stress, delta_eps_p) - data["hmod"] * alpha_new * delta_alpha
    energy_density = 0.5 * voigt_inner(stress, eps_e) + hardening_energy + dissipation
    internal_energy = cfg.thickness * torch.sum(data["areas"] * energy_density)
    return {
        "disp": u_flat,
        "residual": fint,
        "stress": stress,
...
def train_feinn(
    ...
    alpha_prev = torch.zeros(ne, dtype=dtype, device=device)
    history: list[dict[str, float | int]] = []
    final_u = np.zeros(mesh["nodes"].shape[0] * 2, dtype=np.float64)
    final_state = {"stress": np.zeros((ne, 4), dtype=np.float64), "alpha": np.zeros(ne, dtype=np.float64)}
    ...
    for step in range(1, cfg.load_steps + 1):
        ...
        for epoch in range(1, cfg.rprop_epochs + 1):
            ...
            state = evaluate_state(cfg, pred, tdata, eps_p_prev, alpha_prev)
        ...
        with torch.no_grad():
            ...
            final_state = {
                ...
                "alpha": state["alpha"].detach().cpu().numpy(),
            }
            eps_p_prev = state["eps_p"].detach()
            alpha_prev = state["alpha"].detach()
...
def main() -> None:
    ...
    fem_dir = Path(args.fem_dir) if args.fem_dir else base_dir / "outputs" / "perforated_plate_rectangular_fem"
    run_cfg = load_run_config(fem_dir / "perforated_plate_rectangular_fem_run_config.txt")
    cfg = Config(
        fem_dir=str(fem_dir),
        objective_mode=args.objective,
        width=float(run_cfg.get("width", 200.0)),
        height=float(run_cfg.get("height", 100.0)),
    ...
    out_dir = Path(args.output_dir) if args.output_dir else base_dir / "outputs" / f"perforated_plate_rectangular_feinn_baseline_{objective_tag}"
    ...
    save_fields_csv(nodes, fem_fields, feinn_fields, out_dir / "perforated_plate_rectangular_feinn_baseline_fields.csv")
    history.to_csv(out_dir / "perforated_plate_rectangular_feinn_baseline_training_history.csv", index=False)
    plot_panel(nodes, triangles, fem_fields, feinn_fields, out_dir / "perforated_plate_rectangular_feinn_baseline_panel.png")
    plot_uy_figure(nodes, triangles, fem_fields, feinn_fields, out_dir / "perforated_plate_rectangular_feinn_baseline_uy.png")
    with open(out_dir / "perforated_plate_rectangular_feinn_baseline_metrics.json", "w", encoding="utf-8") as f:
        json.dump(summarize_metrics(fem_fields, feinn_fields), f, indent=2)
    with open(out_dir / "perforated_plate_rectangular_feinn_baseline_run_config.json", "w", encoding="utf-8") as f:
        json.dump(asdict(cfg), f, indent=2)

    print(f"Output directory: {out_dir}")
    print("Saved:")
    print("  perforated_plate_rectangular_feinn_baseline_fields.csv")
    print("  perforated_plate_rectangular_feinn_baseline_training_history.csv")
    print("  perforated_plate_rectangular_feinn_baseline_panel.png")
    print("  perforated_plate_rectangular_feinn_baseline_uy.png")
    print("  perforated_plate_rectangular_feinn_baseline_metrics.json")
    print("  perforated_plate_rectangular_feinn_baseline_run_config.json")
