...
def validate_fem_reference_config(run_cfg: dict[str, float | int | str], hardening_mode: str, fem_dir: Path) -> None:
    ref_mode = str(run_cfg.get("hardening_mode", "isotropic"))
    if ref_mode != hardening_mode:
        raise ValueError(
            f"FEM reference hardening mismatch: FEINN requested '{hardening_mode}', "
            f"but {fem_dir} was generated with '{ref_mode}'."
        )
...
def load_fem_dataset(fem_dir: Path) -> tuple[dict[str, np.ndarray], dict[str, np.ndarray]]:
    field_rows = pd.read_csv(fem_dir / "perforated_plate_square_fem_fields.csv")
    elem_rows = pd.read_csv(fem_dir / "perforated_plate_square_fem_elements.csv")
    ...
    mesh = {
        "nodes": nodes,
        "triangles": triangles,
        "left_nodes": load_node_ids(fem_dir / "perforated_plate_square_fem_boundary_left.csv"),
        "bottom_nodes": load_node_ids(fem_dir / "perforated_plate_square_fem_boundary_bottom.csv"),
        "top_nodes": load_node_ids(fem_dir / "perforated_plate_square_fem_boundary_top.csv"),
        "right_nodes": load_node_ids(fem_dir / "perforated_plate_square_fem_boundary_right.csv"),
        "hole_nodes": load_node_ids(fem_dir / "perforated_plate_square_fem_boundary_hole.csv"),
        "top_edges": load_top_edges(fem_dir / "perforated_plate_square_fem_top_edges.csv"),
    }
    fem_fields = {
...
def hardening_suffix(mode: str) -> str:
...
def voigt_j2_norm(v: torch.Tensor) -> torch.Tensor:
    return torch.sqrt(torch.clamp(1.5 * (v[:, 0] ** 2 + v[:, 1] ** 2 + v[:, 2] ** 2 + 2.0 * v[:, 3] ** 2), min=0.0))
...
def isotropic_return_torch(
    ...
    vm_trial = voigt_j2_norm(s_trial)
...
def mixed_hardening_update_torch(
    ...
    f_trial = voigt_j2_norm(s_trial - x_old) - radius_prev
    plastic = f_trial > 1.0e-10
    if not bool(torch.any(plastic)):
        return trial, eps_p_prev, p_prev, x1_prev, x2_prev

    def residual(dpeq: torch.Tensor) -> torch.Tensor:
        p_new = p_prev + dpeq
        ...
        return radius * alpha - voigt_j2_norm(z)

    dpeq = torch.clamp(
        f_trial / (3.0 * mu + cfg.kin_c1 + cfg.kin_c2 + abs(cfg.iso_q1 * cfg.iso_b1)),
        min=0.0,
    )
    for _ in range(80):
        res = residual(dpeq)
        h = torch.clamp(1.0e-6 * torch.maximum(torch.ones_like(dpeq), torch.abs(dpeq)), min=1.0e-8)
        deriv = (residual(dpeq + h) - res) / h
        deriv = torch.where(torch.abs(deriv) < 1.0e-14, torch.ones_like(deriv), deriv)
        dpeq = torch.where(plastic, torch.clamp(dpeq - res / deriv, min=0.0), dpeq)
    ...
    w2 = 1.0 / (1.0 + cfg.kin_gamma2 * dpeq)
    z = s_trial - w1.unsqueeze(1) * x1_prev - w2.unsqueeze(1) * x2_prev
    z_norm = torch.clamp(voigt_j2_norm(z), min=1.0e-12)
    nbar = z / z_norm.unsqueeze(1)
    alpha = 1.0 + (3.0 * mu + w1 * cfg.kin_c1 + w2 * cfg.kin_c2) * dpeq / torch.clamp(radius, min=1.0e-12)
    ...
    s_new = x1_new + x2_new + z / alpha.unsqueeze(1)
    sigma_new = torch.cat(
        [
            s_new[:, 0:1] + mean_trial,
            s_new[:, 1:2] + mean_trial,
    ...
    eps_p_new = eps_p_prev + 1.5 * dpeq.unsqueeze(1) * nbar
    sigma = torch.where(plastic.unsqueeze(1), sigma_new, trial)
    eps_p = torch.where(plastic.unsqueeze(1), eps_p_new, eps_p_prev)
    p_eq = torch.where(plastic, p_new, p_prev)
    x1 = torch.where(plastic.unsqueeze(1), x1_new, x1_prev)
    x2 = torch.where(plastic.unsqueeze(1), x2_new, x2_prev)
    return sigma, eps_p, p_eq, x1, x2
...
def evaluate_state(
    ...
    if cfg.hardening_mode == "kinematic":
        stress, eps_p_new, p_new, x1_new, x2_new = mixed_hardening_update_torch(
            cfg, strain, eps_p_prev, p_prev, x1_prev, x2_prev, data["cmat"], data["mu"]
        )
    else:
        stress, eps_p_new, p_new = isotropic_return_torch(
            cfg, strain, eps_p_prev, p_prev, data["cmat"], data["mu"], data["hmod"]
        )
        x1_new = x1_prev
        x2_new = x2_prev
    ...
    if cfg.hardening_mode == "kinematic":
        kin_c1_safe = torch.clamp(data["kin_c1"], min=1.0e-12)
        kin_c2_safe = torch.clamp(data["kin_c2"], min=1.0e-12)
        hardening_energy = 0.5 * (
            voigt_inner(x1_new, x1_new) / kin_c1_safe + voigt_inner(x2_new, x2_new) / kin_c2_safe
        )
        dissipation = voigt_inner(stress, delta_eps_p)
    else:
...
def residual_metrics(residual: torch.Tensor, free_dofs: torch.Tensor) -> dict[str, torch.Tensor]:
    ...
    ref_norm = torch.clamp(
        torch.sqrt(torch.tensor(float(residual_free.numel()), dtype=residual.dtype, device=residual.device)),
        min=1.0,
    )
...
def train_feinn(
    ...
    for step in range(1, cfg.load_steps + 1):
        ...
        tick = time.time()
        print(
            f"[RPROP] step {step:02d}/{cfg.load_steps} | "
            f"top_uy={target_uy:.6f} | epochs={cfg.rprop_epochs} | lr={cfg.lr}"
        )
        for epoch in range(1, cfg.rprop_epochs + 1):
            ...
            if epoch == 1 or epoch % 200 == 0 or epoch == cfg.rprop_epochs:
                ...
                print(
                    f"[RPROP] step {step:02d} | epoch {epoch:5d}/{cfg.rprop_epochs} | "
                    f"loss={loss_scalar:.6e} | "
                    f"objective={cfg.objective_mode} | "
                    f"potential={float(loss_potential.detach().cpu()):.6e} | "
                    f"residual={float(loss_force.detach().cpu()):.6e} | dt={elapsed:.2f}s"
                )
                history.append(
                    {
                        "load_step": step,
        ...
        with torch.no_grad():
            ...
            final_state = {
                "stress": state["stress"].detach().cpu().numpy(),
                "p_eq": state["p_eq"].detach().cpu().numpy(),
            }
            ...
            print(f"[RPROP] step {step:02d} done | free_res={free_res:.6e}")
...
def postprocess(mesh: dict[str, np.ndarray], state: dict[str, np.ndarray], displacement: np.ndarray) -> dict[str, np.ndarray]:
    ...
    eqp_elem = state["p_eq"][:, None]
    eqp_nodal = nodal_average(nodes.shape[0], triangles, eqp_elem).ravel()
...
def plot_panel(nodes: np.ndarray, triangles: np.ndarray, fem_fields: dict[str, np.ndarray], feinn_fields: dict[str, np.ndarray], out_path: Path) -> None:
    ...
    names = [
        ...
        ("sigma_vm", r"$\sigma_{vm}$"),
        ("eps_p_eq", r"$\bar{\varepsilon}^p$"),
    ]
    fig, axes = plt.subplots(len(names), 3, figsize=(10.2, 17.0), dpi=220)
    ...
    for i, (key, label) in enumerate(names):
        ...
        plot_specs = [
            (fem, fem_vmin, fem_vmax),
            (pred, fem_vmin, fem_vmax),
            (err, err_vmin, err_vmax),
        ]
        for j, (data, vmin, vmax) in enumerate(plot_specs):
            ax = axes[i, j]
...
def plot_uy_figure(nodes: np.ndarray, triangles: np.ndarray, fem_fields: dict[str, np.ndarray], feinn_fields: dict[str, np.ndarray], out_path: Path) -> None:
    tri_obj = mtri.Triangulation(nodes[:, 0], nodes[:, 1], triangles)
    fig, axes = plt.subplots(1, 3, figsize=(12, 3.8), dpi=220, constrained_layout=True)
    fem_uy = fem_fields["uy"]
    pred_uy = feinn_fields["uy"]
    err_uy = pred_uy - fem_uy
    fem_vmin = float(np.min(fem_uy))
    fem_vmax = float(np.max(fem_uy))
    err_vmin = float(np.min(err_uy))
    err_vmax = float(np.max(err_uy))
    plot_specs = [
        ("FEM $u_y$", fem_uy, fem_vmin, fem_vmax),
        ("FEINN $u_y$", pred_uy, fem_vmin, fem_vmax),
        ("Error", err_uy, err_vmin, err_vmax),
    ]
    for ax, (title, field, vmin, vmax) in zip(axes, plot_specs):
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Standalone FEINN von Mises perforated plate using Julia FEM mesh.")
    parser.add_argument("--fem-dir", type=str, default=None)
    parser.add_argument("--output-dir", type=str, default=None)
    parser.add_argument("--objective", type=str, choices=("dem", "dcm"), default="dcm")
    parser.add_argument("--hardening", type=str, choices=("isotropic", "kinematic"), default="isotropic")
...
def main() -> None:
    ...
    fem_dir = Path(args.fem_dir) if args.fem_dir else base_dir / "outputs" / "perforated_plate_square_fem"
    run_cfg = load_run_config(fem_dir / "perforated_plate_square_fem_run_config.txt")
    validate_fem_reference_config(run_cfg, args.hardening, fem_dir)
    cfg = Config(
        fem_dir=str(fem_dir),
        objective_mode=args.objective,
        hardening_mode=args.hardening,
        ...
        top_displacement=float(run_cfg.get("top_displacement", 2.0)),
    ...
    hardening_tag = hardening_suffix(cfg.hardening_mode)
    out_dir = (
        Path(args.output_dir)
        if args.output_dir
        else base_dir / "outputs" / f"perforated_plate_square_feinn_baseline_{hardening_tag}_{objective_tag}"
    )
    out_dir.mkdir(parents=True, exist_ok=True)

    print(f"[Mesh] reading Julia FEM exports from: {fem_dir}")
    mesh, fem_fields = load_fem_dataset(fem_dir)
    nodes = mesh["nodes"]
    triangles = mesh["triangles"]
    print(f"[Mesh] nodes={nodes.shape[0]} | triangles={triangles.shape[0]} | top_nodes={mesh['top_nodes'].size}")

    b_mats, areas = build_tri_operators(nodes, triangles)
    dof_map = build_dof_map(triangles)

    print("[FEINN] training")
    feinn_u, feinn_state, history = train_feinn(cfg, mesh, b_mats, areas, dof_map)
    feinn_fields = postprocess(mesh, feinn_state, feinn_u)

    print("[Post] exporting")
    save_fields_csv(nodes, fem_fields, feinn_fields, out_dir / "perforated_plate_square_feinn_baseline_fields.csv")
    history.to_csv(out_dir / "perforated_plate_square_feinn_baseline_training_history.csv", index=False)
    plot_panel(nodes, triangles, fem_fields, feinn_fields, out_dir / "perforated_plate_square_feinn_baseline_panel.png")
    plot_uy_figure(nodes, triangles, fem_fields, feinn_fields, out_dir / "perforated_plate_square_feinn_baseline_uy.png")
    with open(out_dir / "perforated_plate_square_feinn_baseline_metrics.json", "w", encoding="utf-8") as f:
        json.dump(summarize_metrics(fem_fields, feinn_fields), f, indent=2)
    with open(out_dir / "perforated_plate_square_feinn_baseline_run_config.json", "w", encoding="utf-8") as f:
        json.dump(asdict(cfg), f, indent=2)

    print(f"Output directory: {out_dir}")
    print("Saved:")
    print("  perforated_plate_square_feinn_baseline_fields.csv")
    print("  perforated_plate_square_feinn_baseline_training_history.csv")
    print("  perforated_plate_square_feinn_baseline_panel.png")
    print("  perforated_plate_square_feinn_baseline_uy.png")
    print("  perforated_plate_square_feinn_baseline_metrics.json")
    print("  perforated_plate_square_feinn_baseline_run_config.json")
